...
if __name__ == "__main__":
    ...
    for specific in specific_counter.keys():
        ...
        # set up figures
        fig_loss, ax_loss = plt.subplots(3, 2, figsize=(10,4), sharey="row", sharex=True)
        fig_acc, ax_acc = plt.subplots(nrows, 2, figsize=(10, 10/10*nrows), sharex=True, sharey="row")
        fig_grad, ax_grad = plt.subplots(2, figsize=(10,5), sharex=True)
        ...
        for run in range(1,nruns+1):
            ...
            ### train
            ### losses
            ax_loss[0,0].set_title("Training", fontsize=fontsize)
            # train loss
            ax_loss[0,0].loglog(median.index +1, median['train_loss'], c=color, label=label)
            ax_loss[0,0].fill_between(median.index +1, q05['train_loss'], q95['train_loss'], alpha=0.5, color=color)
            ax_loss[0,0].set_ylabel('Loss', fontsize=fontsize)
            ax_loss[0,0].grid(True)
            # train loss Favoured
            ax_loss[1,0].loglog(median.index +1, median[f'train_loss_class_{0}'], c=color)
            ax_loss[1,0].fill_between(median.index +1, q05[f'train_loss_class_{0}'], q95[f'train_loss_class_{0}'], alpha=0.5, color=color)
            ax_loss[1,0].set_ylabel(f'Fav. Class', fontsize=fontsize)
            ax_loss[1,0].grid(True)
            # train loss Unfavoured
            ax_loss[2,0].loglog(median.index +1, median[f'train_loss_class_{n_classes-1}'], c=color)
            ax_loss[2,0].fill_between(median.index +1, q05[f'train_loss_class_{n_classes-1}'], q95[f'train_loss_class_{n_classes-1}'], alpha=0.5, color=color)
            ax_loss[2,0].set_ylabel(f'Unfav. Class', fontsize=fontsize)
            ax_loss[2,0].grid(True)
            ...
            # train accuracy Favoured
            ax_acc[1,0].semilogx(median.index +1, median[f'max_train_acc'], c=color)
            ax_acc[1,0].fill_between(median.index +1, q05[f'max_train_acc'], q95[f'max_train_acc'], alpha=0.5, color=color)
            ax_acc[1,0].set_ylabel(f'Fav.', fontsize=fontsize)
            ax_acc[1,0].grid(True)
            # train accuracy Unfavoured
            ax_acc[2,0].semilogx(median.index +1, median[f'min_train_acc'], c=color)
            ax_acc[2,0].fill_between(median.index +1, q05[f'min_train_acc'], q95[f'min_train_acc'], alpha=0.5, color=color)
            ax_acc[2,0].set_ylabel(f'Unfav.', fontsize=fontsize)
            ax_acc[2,0].grid(True)
            if plot_df:
                # delta fr
                if n_classes < 10:
                    ax_acc[3,0].semilogx(median.index +1, median[f'max_train_freq'], c=color)
                    ax_acc[3,0].set_ylim(1/n_classes*0.9,1)
                else:
                    ax_acc[3,0].loglog(median.index +1, median[f'max_train_freq'], c=color)
                    ax_acc[3,0].set_ylim(1/n_classes*0.5,1)
                ax_acc[3,0].fill_between(median.index +1, q05[f'max_train_freq'], q95[f'max_train_freq'], alpha=0.5, color=color)
                ax_acc[3,0].set_ylabel(f'Max Freq.', fontsize=fontsize)
                ax_acc[3,0].axhline(1/n_classes, color="green")
                ax_acc[3,0].grid(True)
                

            # test
            ### losses
            ax_loss[0,1].set_title("Test", fontsize=fontsize)
            # test loss
            ax_loss[0,1].loglog(median.index +1, median['test_loss'], c=color, label=label)
            ax_loss[0,1].fill_between(median.index +1, q05['test_loss'], q95['test_loss'], alpha=0.5, color=color)
            #ax_loss[0,1].set_ylabel('Test loss')
            ax_loss[0,1].grid(True)
            # test loss Favoured
            ax_loss[1,1].loglog(median.index +1, median[f'test_loss_class_{0}'], c=color)
            ax_loss[1,1].fill_between(median.index +1, q05[f'test_loss_class_{0}'], q95[f'test_loss_class_{0}'], alpha=0.5, color=color)
            #ax_loss[1,1].set_ylabel(f'Favoured Class Test Loss')
            ax_loss[1,1].grid(True)
            # test loss Unfavoured
            ax_loss[2,1].loglog(median.index +1, median[f'test_loss_class_{n_classes-1}'], c=color)
            ax_loss[2,1].fill_between(median.index +1, q05[f'test_loss_class_{n_classes-1}'], q95[f'test_loss_class_{n_classes-1}'], alpha=0.5, color=color)
            #ax_loss[2,1].set_ylabel(f'Unfavoured Class Test Loss')
            ax_loss[2,1].grid(True)
            ...
            #ax_acc[0,1].set_ylabel('Test Acc.')
            ax_acc[0,1].grid(True)
            # test accuracy Favoured
            ax_acc[1,1].semilogx(median.index +1, median[f'max_test_acc'], c=color)
            ax_acc[1,1].fill_between(median.index +1, q05[f'max_test_acc'], q95[f'max_test_acc'], alpha=0.5, color=color)
            #ax_acc[1,1].set_ylabel(f'Test Acc. Favoured Class')
            ax_acc[1,1].grid(True)
            # test accuracy Unfavoured
            ax_acc[2,1].semilogx(median.index +1, median[f'min_test_acc'], c=color)
            ax_acc[2,1].fill_between(median.index +1, q05[f'min_test_acc'], q95[f'min_test_acc'], alpha=0.5, color=color)
            #ax_acc[2,1].set_ylabel(f'Test Acc. Unfavoured Class')
            ax_acc[2,1].grid(True)
            if plot_df:
                # delta fr
                if n_classes <10:
                    ax_acc[3,1].semilogx(median.index +1, median[f'max_test_freq'], c=color)
                    ax_acc[3,1].set_ylim(1/n_classes*0.9,1)
                else:
                    ax_acc[3,1].loglog(median.index +1, median[f'max_test_freq'], c=color)
                    ax_acc[3,1].set_ylim(1/n_classes*0.5,1)
                ax_acc[3,1].fill_between(median.index +1, q05[f'max_test_freq'], q95[f'max_test_freq'], alpha=0.5, color=color)
                #ax_acc[3,1].set_ylabel(f'Max Test Frequency')
                ax_acc[3,1].axhline(1/n_classes, color="green")
                ax_acc[3,1].grid(True)
                

            ### gradients
            # test gradient Favoured
            is_grad = [True for col in median.columns.astype(str) if "grads" in col]
            is_grad = True if len(is_grad)>0 else False
            if is_grad:
                ax_grad[0].loglog(median.index +1, median[f'grads_5q_class_{0}'], c=color, label=label)
                ax_grad[0].fill_between(median.index +1, q05[f'grads_5q_class_{0}'], q95[f'grads_5q_class_{0}'], alpha=0.5, color=color)
                ax_grad[0].set_ylabel(f'Fav. Class', fontsize=fontsize)
                ax_grad[0].grid(True)
                # test gradient Unfavoured
                ax_grad[1].loglog(median.index +1, median[f'grads_5q_class_{n_classes-1}'], c=color)
                ax_grad[1].fill_between(median.index +1, q05[f'grads_5q_class_{n_classes-1}'], q95[f'grads_5q_class_{n_classes-1}'], alpha=0.5, color=color)
                ax_grad[1].set_ylabel(f'Unfav. Class', fontsize=fontsize)
                ax_grad[1].grid(True)   
        ...
        ncol = 3 if act_func=="ReLU" else 4
        
        handles, labels = ax_loss[0,0].get_legend_handles_labels()
        fig_loss.supxlabel('Number of training steps', fontsize=fontsize)
        fig_loss.tight_layout(rect=[0, 0, 1, 0.95])
        legend = fig_loss.legend(handles, labels,handletextpad=0.5, loc="upper center",bbox_to_anchor=(0.5, 0.99), ncol=ncol, frameon=True,fontsize=10)
        for legline in legend.get_lines():
            legline.set_linewidth(4)  # make legend lines thicker
        fig_loss.savefig(os.path.join("figures", "train", f'{model}_{act_func}_{dataset}_loss.png'), dpi=300)
        handles, labels  = ax_acc[0,0].get_legend_handles_labels()
        
        fig_acc.supxlabel('Number of training of steps', fontsize=fontsize)
        fig_acc.tight_layout(rect=[0, 0, 1, 0.95])
        legend = fig_acc.legend(handles, labels, handletextpad=0.5, loc="upper center",bbox_to_anchor=(0.5, 1.0), ncol=ncol,frameon=True, fontsize=10)
        for legline in legend.get_lines():
            legline.set_linewidth(4)  # make legend lines thicker
        fig_acc.savefig(os.path.join("figures", "train", f'{model}_{act_func}_{dataset}_accuracy.png'), dpi=300)

        handles, labels  = ax_grad[0].get_legend_handles_labels()
        fig_grad.supxlabel('Number of training steps', fontsize=fontsize)
        fig_grad.tight_layout(rect=[0, 0, 1, 0.97])
        legend = fig_grad.legend(handles, labels,handletextpad=0.5,loc="upper right", bbox_to_anchor=(1, 0.97), ncol=ncol, frameon=True, fontsize=10)
        for legline in legend.get_lines():
            legline.set_linewidth(4)  # make legend lines thicker
        fig_grad.savefig(os.path.join("figures", "train", f'{model}_{act_func}_{dataset}_gradients.png'), dpi=300)